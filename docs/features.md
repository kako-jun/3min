...
**記号**と**スタンプ**の2種類があり、同時に設定可能（排他ではない）。

### 記号（背景表示）

セル全体に薄く大きく表示される。空き状況を視覚的に表現。
...
- 月の全日を1本のスクロールリストで表示（広い/狭い画面問わず統一）
- リスト高さは画面下端まで動的に伸ばす（背の高い端末では多く、短い端末では少なく見える。カレンダーと最小リスト高が収まる端末ではコメント入力欄が画面内に残る。極端に低い画面では最小高を確保しページスクロールになる）
...
カレンダー上部のボタンから、先月のデータを今月にコピーできる。

### 動作仕様

- 曜日ごとに最頻出のパターンを推測して適用
- **上書き動作**: 今月の既存データは消えた上でコピーされる（マージではない）
- **全曜日を再現**: 先月に予定がない曜日は、今月もクリアされる
- コピー対象フィールド:
  - **text**: 予定コメント
  - **symbol**: 背景記号（◯/△/✕）
  - **stamp**: コーナースタンプ（満/休）
  - **timeFrom / timeTo**: 営業時間
- 例: 先月の月曜日に「10:00-18:00」が多ければ、今月の全月曜日に同じ時間を設定

### ユースケース

- 定休日パターンの引き継ぎ（毎週水曜休み等）
- 営業時間の一括設定
- 月初めの入力作業を効率化
